#!/usr/bin/env bash
#
# Corvex-RedOps — one-command installer & deployer
# ---------------------------------------------
...
# SaaS stack (Corvex-RedOps API + web console + mobile-analysis engine) with a single command.
#
...
log()  { printf "${c_blue}[Corvex-RedOps]${c_off} %s\n" "$*"; }
...
SaaS stack (Corvex-RedOps API + web console + mobile-analysis engine).

Usage:
  sudo ./install.sh              install prerequisites + deploy
  sudo ./install.sh --port 9090  deploy on a custom console port
  sudo ./install.sh --ai         deploy WITH the local AI layer
...
ask_ai() {
  ...
  printf "\n${c_blue}[Corvex-RedOps]${c_off} Optional AI layer (local, self-hosted AI model).\n"
...
wait_ollama() {
  log "Waiting for the local AI runtime to become ready…"
  for _ in $(seq 1 40); do
    if curl -fsS "http://localhost:11434/api/tags" >/dev/null 2>&1; then ok "Local AI runtime is up."; return 0; fi
    sleep 3
  done
  warn "The local AI runtime did not become ready in time — you can pull the model later with:"
...
write_env() {
  ...
  cat > "$ENV_FILE" <<ENV
# Corvex-RedOps deployment secrets — generated by install.sh. Keep private; do not commit.
...
# ----------------------------------------------------------------------------- deploy
deploy() {
  [ -f "$COMPOSE_FILE" ] || die "compose file not found: $COMPOSE_FILE (run this from the Corvex-RedOps repo)"
  ...
  local profile=""; [ "${AI_CHOICE:-0}" = "1" ] && profile="--profile ai" && log "AI layer enabled — the local AI runtime will start too."
...
wait_healthy() {
  log "Waiting for the Corvex-RedOps API to become healthy…"
  for _ in $(seq 1 40); do
    if curl -fsS "http://localhost:${PORT}/health" >/dev/null 2>&1; then ok "Corvex-RedOps is up."; return 0; fi
    sleep 3
  done
...
summary() {
  ...
  [ -n "$ip" ] || ip="localhost"
  printf "\n${c_grn}════════════════════════════════════════════════════════════${c_off}\n"
  printf "  ${c_grn}✔ Corvex-RedOps is deployed.${c_off}\n"
  printf "${c_grn}════════════════════════════════════════════════════════════${c_off}\n"
  printf "  Web console : ${c_blue}http://localhost:%s${c_off}   (or http://%s:%s)\n" "$PORT" "$ip" "$PORT"
  printf "  API docs    : ${c_blue}http://localhost:%s/docs${c_off}\n" "$PORT"
  printf "  Mobile      : http://localhost:8000  ${c_dim}(mobile analysis engine)${c_off}\n"
  if [ "${AI_CHOICE:-0}" = "1" ]; then
    printf "  AI layer    : ${c_grn}enabled${c_off} — local model ${c_blue}%s${c_off} via the local AI runtime (http://localhost:11434)\n" "$AI_MODEL"
  else
    printf "  AI layer    : ${c_dim}disabled (re-run with --ai to enable)${c_off}\n"
  fi
  printf "\n  Next steps:\n"
  printf "    1) Sign in as ${c_blue}admin${c_off}. A one-time password was generated on first run —\n"
  printf "       read it from the API log:\n"
  printf "         ${c_dim}%s --env-file %s -f %s logs %s 2>&1 | grep -A3 'first-run admin'${c_off}\n" "$DC" "$ENV_FILE" "$COMPOSE_FILE" "pentestiq"
  printf "       ${c_dim}(pin your own with CORVEX_ADMIN_USER / CORVEX_ADMIN_PASSWORD in %s). Change it after first login.${c_off}\n" "$ENV_FILE"
...
main() {
  ...
  if [ "$(uname -s)" = "Darwin" ] && [ "$(id -u)" = "0" ] && [ -n "${SUDO_USER:-}" ] && [ "${SUDO_USER}" != "root" ]; then
    printf "[Corvex-RedOps] macOS uses Colima (per-user Docker) — re-running as %s (sudo not needed here)…\n" "${SUDO_USER}"
